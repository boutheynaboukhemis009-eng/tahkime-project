...
const app = express();
const PORT = process.env.PORT || 3000; // مضاف للتوافق الديناميكي مع منافذ استضافة Render
...
app.use(express.urlencoded({ extended: true })); // مضاف لضمان قراءة بيانات الـ Forms المستلمة بالكامل
 ...
 * [تعديل مسار] تم تحويل المسار إلى المجلد الرئيسي (__dirname) مباشرة ليتوافق مع هيكلة ملفاتك على جيت هوب ولحل مشكلة اختفاء التنسيق
 */
...
// مسار الصفحة الرئيسية الافتراضي لضمان التوجيه للمجلد الحالي مباشرة عند فتح النطاق
app.get('/', (req, res) => {
...
/**
 * 6. مسار توجيه الصفحات العام الديناميكي
 * تم تعديله لضمان عدم تعارضه مع الملفات الثابتة (مثل style.css)
 */
app.get('/:page', (req, res, next) => {
    const page = req.params.page;
    
    // التأكد من أننا نطلب ملف HTML فقط لتجنب تداخل المسارات
    if (page.endsWith('.html')) {
        res.sendFile(path.join(__dirname, page));
    } else {
        // إذا لم يكن ملف HTML، انتقل للمسار التالي (ليقوم express.static بخدمة الملف)
        next();
    }
});
